...
- Body 保存上限支持 `0` 表示不截断保存。
- 管理端 Token 认证，代理路径不需要 Token。
- 数据库可视化页面和只读 SQL 查询下载。
...
- [x] Body 保存上限支持 `0` 表示不截断，并迁移旧默认 256KB 规则。
- [x] 管理页面和管理 API 增加 Token 认证，代理转发路径放行。
- [x] 新增数据库页：表结构、只读 SQL 查询、JSON/CSV 下载。
